...
public class FileNameParser : IFileNameParser
{
    ...
    private readonly Regex regexPhoto = new(@"^photo_(\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})$", RegexOptions.Compiled);
    ...
    public DateTime? ExtractDateFromFileName(string fileName)
    {
        ...
        }else if (regexPhoto.Match(fileNameWithoutExtension).Success)
        {
            var dateComponent = regexPhoto.Match(fileNameWithoutExtension).Groups[1].Value;
            if (DateTime.TryParseExact(dateComponent, "yyyy-MM-dd_HH-mm-ss", CultureInfo.InvariantCulture,
                    DateTimeStyles.None, out var date))
            {
                return date;
            }
